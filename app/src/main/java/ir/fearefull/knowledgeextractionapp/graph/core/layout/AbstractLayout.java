...
abstract class AbstractLayout implements Layout {
    ...
    protected AbstractLayout(Graph graph, Transformer<Node, Point2D> initializer) {
        ...
        this.locations
                = LazyMap.lazyMap(new HashMap<Node, Point2D>(), chain);
        initialized = true;
    }
     ...
     * @param size        the size
     */
    AbstractLayout(Graph graph, Transformer<Node, Point2D> initializer, Dimension size) {
        this.graph = graph;
    ...
    private void adjustLocations(Dimension oldSize, Dimension size) {
        ...
        while (true) {
            try {
                ...
                Timber.tag(AbstractLayout.class.getName()).e(cme);
            }
        }
    }
